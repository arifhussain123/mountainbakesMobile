...
function boot() {
  jest.resetModules();
  return {
    writeOffline: require('@/common/database/repositories/offlineWriteRepository').writeOffline,
    queue: require('@/common/database/repositories/syncQueueRepository'),
    drainQueue: require('@/api/sync/syncManager').drainQueue,
  };
}

async function migrate(db: TestDb): Promise<void> {
  const { runMigrations } = require('@/common/database/runMigrations');
