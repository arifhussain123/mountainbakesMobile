...
describe('navigation surface', () => {
  ...
  it('never exposes one destination from both a tab and More', () => {
    for (const role of USER_ROLES) {
      ...
      const moreRoutes = moreSectionsFor(profile)
        .flatMap(s => s.items)
   ...
   * server does.
   */
  it('gives a branch manager Home · Orders · Sales · Stock, then More', () => {
    const tabs = tabsFor(profileFor('branch_manager'));
    expect(tabs.map(t => t.name)).toEqual(['Home', 'Orders', 'Sales', 'Stock', 'More']);
  });
  ...
  it('withholds Home and Reports from a shift account, which the API 403s', () => {
    ...
    expect(tabsFor(shift).map(t => t.name)).toEqual(['Orders', 'Sales', 'Stock', 'More']);
    expect(landingTabFor(shift)).toBe('Orders');
  ...
  /**
   * The rule the whole restructure exists to hold: the account panel and More
   * share nothing.
   *
   * This is the check that was missing, and its absence shipped a real bug.
   * The old single-path test compared More routes against *tab names* only, so
   * sign-out sat in the account panel and in More at the same time — two paths,
   * two separately-written confirms, and the panel's had no confirm at all. It
   * dropped the session without ever mentioning that the queue still held the
   * only copy of a transaction.
   *
   * `ACCOUNT_PANEL` is the panel's declared inventory. Nothing in it may be a
   * More row or a tab for any role.
   */
  it('shares nothing between the account panel, the tabs and More', () => {
    for (const role of USER_ROLES) {
      const profile = profileFor(role);
      ...
      for (const entry of ACCOUNT_PANEL) {
        expect({ role, entry, inMore: moreKeys.has(entry) }).toEqual({
          role,
          entry,
          inMore: false,
        });
        expect({ role, entry, isATab: tabNames.has(entry) }).toEqual({
          role,
          entry,
          isATab: false,
        });
