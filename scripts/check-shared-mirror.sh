...
# mountainbakes-server/src/shared/, which the web app also mirrors. Nothing
...
SERVER_SHARED="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/mountainbakes-server/src/shared"
...
if diff -r "$SERVER_SHARED" "$MOBILE_SHARED" > /tmp/mb-shared-diff.txt 2>&1; then
  echo "OK: src/shared is identical to mountainbakes-server/src/shared"
  exit 0
fi

echo "DRIFT: src/shared has diverged from mountainbakes-server/src/shared"
echo
cat /tmp/mb-shared-diff.txt
echo
echo "Re-sync with:"
echo "  rm -rf src/shared && cp -r ../mountainbakes-server/src/shared src/shared"
echo "Then verify the web app's copy matches too:"
echo "  diff -r ../mountainbakes-server/src/shared ../mountainbakes-frontend/src/shared"
exit 1
