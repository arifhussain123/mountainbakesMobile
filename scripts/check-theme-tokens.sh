...
# renders one theme.
#
# src/common/theme/ is exempt: that is where the literals belong.
set -euo pipefail

cd "$(dirname "$0")/.."

if hits=$(grep -rnE '#[0-9a-fA-F]{3,8}\b|\brgba?\(' src/common/ui \
    --include='*.ts' --include='*.tsx' \
    | grep -v '__tests__'); then
  echo "Hard-coded colours in src/common/ui — use a theme token:" >&2
  echo "$hits" >&2
  exit 1
fi

echo "OK: every colour in src/common/ui comes from the theme"
